	...
	<div>
		<c:if test="${currentPage > 0 and currentPage <= endPage}">
			...
			<c:if test="${endPage - currentPage <= 0 and endPage > 4}">
				<a class="page-link" href="${pageContext.request.contextPath}/employee/teacher/teacherList?currentPage=${currentPage-4}&searchWord=${searchWord}">${currentPage-4}</a>
			</c:if>		    	
			<c:if test="${endPage - currentPage <= 1 and endPage > 3}">
		    	<a class="page-link" href="${pageContext.request.contextPath}/employee/teacher/teacherList?currentPage=${currentPage-3}&searchWord=${searchWord}">${currentPage-3}</a>
	    	</c:if>		    	
			<c:if test="${currentPage >= 3}">
				<a class="page-link" href="${pageContext.request.contextPath}/employee/teacher/teacherList?currentPage=${currentPage-2}&searchWord=${searchWord}">${currentPage-2}</a>
	    	</c:if>		    	
			<c:if test="${currentPage >= 2}">
				<a class="page-link" href="${pageContext.request.contextPath}/employee/teacher/teacherList?currentPage=${currentPage-1}&searchWord=${searchWord}">${currentPage-1}</a>
	    	</c:if>
			<c:if test="${currentPage > 0}">
				<a class="page-link" style="color: red" href="${pageContext.request.contextPath}/employee/teacher/teacherList?currentPage=${currentPage}&searchWord=${searchWord}">${currentPage}</a>
	    	</c:if>
			...
			<a href="${pageContext.request.contextPath}/employee/teacher/teacherList?currentPage=${endPage}&searchWord=${searchWord}">끝으로</a>
		</c:if>
	</div>
</body>
</html>